...
window.UI=(()=>{
 ...
 function saveList(){if(!$('#search'))return;const old=listState();set(viewKey,{...old,search:$('#search').value,week:$('#week-filter').value,sort:$('#sort').value,subject:$('.chip.active')?.dataset.subject||'',tasksOnly:$('#tasks-only')?.getAttribute('aria-pressed')==='true',visibleLimit:Number($('#list')?.dataset.visibleLimit)||40,y:scrollY})}
 ...
 function readerState(){return storage.get('lecture-position:'+readerId)||get('lecture-position:'+readerId)}
 function saveReader(){if(!readerId||!$('#reading-text')||!readingReady)return;const old=readerState();const value={...old,mode:readerMode,[readerMode]:scrollY};set('lecture-position:'+readerId,value);storage.set('lecture-position:'+readerId,value);storage.set('lecture-last-read',{id:readerId,mode:readerMode,at:Date.now()})}
 function readerRendered(id,mode){
  const first=readerId!==id;readerId=id;readerMode=mode;readingReady=false;
  if(!$('#reading-actions')){
  ...
  const saved=readerState();if(first&&!['lecture','full','assignment','extra'].includes(new URLSearchParams(location.search).get('mode'))&&saved.mode&&saved.mode!==mode){const tab=$(`[data-mode="${['lecture','full','assignment','extra'].includes(saved.mode)?saved.mode:'lecture'}"]`);if(tab){tab.click();return}}
 ...
 function searchState(value){const box=$('.search-field'),clear=$('#clear-search');if(!box||!clear)return;const filled=!!value;box.classList.toggle('has-query',filled);clear.hidden=!filled}
 function searchResults(){
  ...
  searchAnimations.push(list.animate([{opacity:.65,transform:'translateY(3px)'},{opacity:1,transform:'translateY(0)'}],{duration:220,easing:'cubic-bezier(.22,1,.36,1)'}));

 }
 return {searchState,searchResults,listState,restoreList,listRendered,announce,skeleton,ready,readerRendered,saveReader};
})();
