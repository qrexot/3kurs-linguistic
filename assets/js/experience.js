...
window.UI=(()=>{
 ...
 function readerRendered(id,mode){
  ...
  const saved=readerState();if(first&&!['lecture','full','assignment'].includes(new URLSearchParams(location.search).get('mode'))&&saved.mode&&saved.mode!==mode){const tab=$(`[data-mode="${['lecture','full','assignment'].includes(saved.mode)?saved.mode:'lecture'}"]`);if(tab){tab.click();return}}
 ...
 function searchResults(){
  ...
  searchAnimations.push(list.animate([{opacity:.78},{opacity:1}],{duration:160,easing:'ease-out'}));
