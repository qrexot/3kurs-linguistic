...
(async()=>{
 ...
 function card(l,i){
  ...
  UI.searchState(search);const filtered=!!(search.trim()||week||selected);$('#reset-filters').hidden=!filtered;$('#list-caption').hidden=filtered;
 ...
 function drawList(animate=false){
  ...
  let list=lectures.filter(l=>{
   ...
   let indexed=searchCache.get(l);if(!indexed){indexed={lecture:cleanText(l.lecture_text),full:cleanText(l.full_text)};searchCache.set(l,indexed)}
   const cleanLec=indexed.lecture;
   const cleanFull=indexed.full;
 ...
 function privacy(){$('#main').innerHTML=`<a class="back-link" href="index.html">${icon('back')} К материалам</a><div class="read-layout"><article class="prose"><h1>О сайте</h1><p>Сайт разработан его высочеством Леонардом, повелителем конспектов, хранителем лекций, укротителем дедлайнов, верховным магистром недосыпа, защитником потерянных записей и великим князем «я потом всё выучу».</p><p>Всё это создано чтобы лекции не терялись где-то в переписках, а перед экзаменом можно было открыть один сайт и хотя бы сделать вид что ситуация под контролем.</p><p>Пользуйтесь на здоровье, его высочество тоже пока не всё выучило.</p></article></div>`}
 ...
 async function buildFingerprint(){
  ...
  // Touch
  parts.push(String('ontouchstart' in window||navigator.maxTouchPoints>0));
  // Canvas fingerprint
  try{
  ...
  // Hash (простой FNV-1a для совместимости без SubtleCrypto проблем)
 ...
 /* ─── Получение IP через ip-api ─── */
 async function fetchGeo(){
  try{
   const r=await fetch('https://ip-api.com/json/?fields=status,query,country,countryCode,city,isp',{signal:AbortSignal.timeout(5000)});
   if(!r.ok)return{ip:'',country:'',countryCode:'',city:'',isp:''};
   const d=await r.json();
   if(d.status!=='success')return{ip:'',country:'',countryCode:'',city:'',isp:''};
   return{ip:d.query||'',country:d.country||'',countryCode:d.countryCode||'',city:d.city||'',isp:d.isp||''};
  }catch(e){return{ip:'',country:'',countryCode:'',city:'',isp:''}}
 }
 ...
 try{
  ...
  try{await track()}catch(e){toast('Материалы открыты. Статистика временно недоступна.')}
