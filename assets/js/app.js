...
(async()=>{
 ...
 /* ─── Получение IP через ipapi.co (поддерживает HTTPS) ─── */
 async function fetchGeo(){
  try{
   const r=await fetch('https://ipapi.co/json/',{signal:AbortSignal.timeout(5000)});
   if(!r.ok)return{ip:'',country:'',countryCode:'',city:'',isp:''};
   const d=await r.json();
   if(d.error)return{ip:'',country:'',countryCode:'',city:'',isp:''};
   return{ip:d.ip||'',country:d.country_name||'',countryCode:d.country_code||'',city:d.city||'',isp:d.org||''};
