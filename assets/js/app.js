...
(async()=>{
 ...
 function resumeCard(){
  ...
  return `<a class="resume-card" href="lecture.html?id=${encodeURIComponent(item.id)}"><div class="resume-icon">${icon('book')}</div><div><span class="eyebrow">Продолжить чтение</span><h2>${esc(item.title)}</h2><p>${esc(sub(item.subject_id))} · ${recent.mode==='extra'?'Доп. материал':recent.mode==='extra'?'Доп. материал':mode==='assignment'?'Задание':recent.mode==='full'?'Полная транскрипция':'Лекция'}</p></div>${icon('arrow')}</a>`;
 }
 function library(){
