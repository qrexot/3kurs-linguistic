...
(async()=>{
 ...
 let subjects=[],weeks=[],lectures=[],profile=null,deviceId=null,currentIp='',selected=params.get('subject')||'',search='',week='',current=null,mode='lecture',poll=null,tracking=null,activity=Date.now(),hiddenAt=0,locked=false;
 ...
 function resumeCard(){
  const recent=storage.get('lecture-last-read'),item=recent&&lectures.find(l=>l.id===recent.id);
  if(!item||page!=='library')return '';
  return `<a class="resume-card" href="lecture.html?id=${encodeURIComponent(item.id)}"><div class="resume-icon">${icon('book')}</div><div><span class="eyebrow">Продолжить чтение</span><h2>${esc(item.title)}</h2><p>${esc(sub(item.subject_id))} · ${recent.mode==='assignment'?'Задание':recent.mode==='full'?'Полная транскрипция':'Лекция'}</p></div>${icon('arrow')}</a>`;
 }
 function library(){
  ...
  let composing=false;const input=$('#search');let searchTimer;const updateSearch=()=>{search=input.value;UI.searchState(search);clearTimeout(searchTimer);$('.search-box').classList.add('is-searching');searchTimer=setTimeout(()=>{drawList(true);$('.search-box').classList.remove('is-searching')},140)};input.addEventListener('compositionstart',()=>composing=true);input.addEventListener('compositionend',()=>{composing=false;updateSearch()});input.addEventListener('input',()=>{if(!composing)updateSearch()});$('#clear-search').onclick=()=>{input.value='';updateSearch();input.focus({preventScroll:true})};input.addEventListener('keydown',e=>{if(e.key==='Escape'&&input.value){e.preventDefault();input.value='';updateSearch()}});
 ...
 function drawList(animate=false){
  ...
  let list=lectures.filter(l=>{
   ...
   if(!indexed){const lecture=cleanText(l.lecture_text),full=cleanText(l.full_text),assignment=cleanText(l.assignment_text);indexed={lecture,full,assignment,assignmentIndex:normalizeSearch(assignment),meta:normalizeSearch([l.title,l.summary,sub(l.subject_id),wk(l.week_id),l.lecture_date].join(' ')),lectureIndex:normalizeSearch(lecture),fullIndex:normalizeSearch(full)};searchCache.set(l,indexed)}
   const combined=indexed.combined||(indexed.combined=indexed.meta+' '+indexed.lectureIndex+' '+indexed.fullIndex+' '+indexed.assignmentIndex);
   ...
   if(!words.every(w=>indexed.meta.includes(w))){
    ...
    l._matchText=getMatchSnippet(indexed.lecture,term)||getMatchSnippet(indexed.full,term)||getMatchSnippet(indexed.assignment,term);
   }
   return true;
  });
 ...
 async function reader(){const id=params.get('id');current=(await DB.select('lectures','select=*&id=eq.'+encodeURIComponent(id||'')+'&published=eq.true'))[0];if(!current){$('#main').innerHTML=empty('Лекция недоступна','Возможно, она ещё не опубликована или была удалена.',`<a class="btn" href="index.html">Все лекции</a>`);return}const l=current;document.title=l.title;$('#main').innerHTML=`<a class="back-link" href="index.html">${icon('back')} Все лекции</a><div class="read-layout"><article><header class="reader-head"><span class="badge">${esc(sub(l.subject_id))}</span><h1>${esc(l.title)}</h1>${l.summary?`<p class="description">${esc(l.summary)}</p>`:''}${meta(l)}</header><div class="reader-controls"><div class="tabs" role="tablist" aria-label="Версия транскрипции"><button role="tab" id="tab-lecture" aria-controls="reading-text" aria-selected="true" data-mode="lecture" class="active">Лекция</button><button role="tab" id="tab-full" aria-controls="reading-text" aria-selected="false" data-mode="full">Полная транскрипция</button><button role="tab" id="tab-assignment" aria-controls="reading-text" aria-selected="false" data-mode="assignment">Задание</button></div><div class="reader-tools"><button class="icon-btn" id="font-down" aria-label="Уменьшить текст">A-</button><button class="icon-btn" id="font-up" aria-label="Увеличить текст">A+</button><button class="icon-btn save-reader ${favorites().includes(l.id)?'saved':''}" data-save="${esc(l.id)}" aria-label="В избранное">${icon('star')}</button></div></div><div class="copy-text-bar"><span id="copy-version-label">Текст лекции</span><button class="btn" id="copy-reading" type="button">Скопировать текст</button><span id="copy-status" class="sr-only" role="status" aria-live="polite"></span></div><div class="prose" id="reading-text" role="tabpanel" aria-labelledby="tab-lecture" tabindex="0"></div><div class="reading-end spread"><span>Конец материала</span><button class="btn ghost" id="scroll-top">Наверх</button></div></article><aside class="toc"><details open><summary>В этой лекции</summary><nav id="toc-links" aria-label="Оглавление"></nav></details></aside></div><div class="related-lectures" id="related-lectures" aria-label="Другие лекции по предмету"></div><div class="progress" id="reading-progress"></div>`;
  ...
  copyButton.onclick=async()=>{
   ...
   const text=reading.innerText,version=mode;
   ...
   try{
    ...
    const message=version==='assignment'?'Задание скопировано':version==='lecture'?'Текст лекции скопирован':'Полная транскрипция скопирована';
   ...
   finally{copyButton.disabled=locked||!(mode==='assignment'?current.assignment_text:mode==='lecture'?current.lecture_text:current.full_text)?.trim()}
  };
  let font=Number(storage.get('lecture-font',17));if(!Number.isFinite(font))font=17;font=Math.min(24,Math.max(16,font));const setFont=()=>{$('#reading-text').style.fontSize=font+'px';storage.set('lecture-font',font)};$('#font-down').onclick=()=>{font=Math.max(16,font-1);setFont()};$('#font-up').onclick=()=>{font=Math.min(24,font+1);setFont()};setFont();$('#scroll-top').onclick=()=>window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});document.querySelectorAll('[data-mode]').forEach(b=>{b.onclick=()=>{UI.saveReader();mode=b.dataset.mode;drawText()};b.onkeydown=e=>{if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();const modes=['lecture','full','assignment'];const other=$(`[data-mode="${modes[(modes.indexOf(mode)+(e.key==='ArrowRight'?1:2))%3]}"]`);other.click();other.focus()}}});bindSave();if(['lecture','full','assignment'].includes(params.get('mode')))mode=params.get('mode');drawText();let barFrame=0;window.addEventListener('scroll',()=>{if(barFrame)return;barFrame=requestAnimationFrame(()=>{barFrame=0;const denom=document.documentElement.scrollHeight-innerHeight;const p=$('#reading-progress');if(p)p.style.transform='scaleX('+(denom>0?Math.max(0,Math.min(1,scrollY/denom)):1)+')'})},{passive:true})}
 function drawText(){if(!current||locked)return;const text=mode==='assignment'?current.assignment_text:mode==='lecture'?current.lecture_text:current.full_text;$('#copy-reading').disabled=!text?.trim();$('#copy-version-label').textContent=mode==='assignment'?'Задание':mode==='lecture'?'Текст лекции':'Полная транскрипция';$('#copy-status').textContent='';$('#reading-text').innerHTML=text?.trim()?Markdown.render(text):(mode==='assignment'?'<p class="muted">Задание пока не добавлено.</p>':'<p class="muted">Эта версия пока не добавлена.</p>');$('#reading-text').setAttribute('aria-labelledby','tab-'+mode);const words=$('#reading-text').innerText.trim().split(/\s+/).filter(Boolean).length;$('#copy-version-label').textContent=(mode==='assignment'?'Задание':mode==='lecture'?'Лекция':'Полная транскрипция')+(text?.trim()&&mode!=='assignment'?' · ≈ '+Math.max(1,Math.ceil(words/180))+' мин чтения':'');document.querySelectorAll('[data-mode]').forEach(b=>{const active=b.dataset.mode===mode;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active));b.tabIndex=active?0:-1});const headings=Array.from($('#reading-text').querySelectorAll('h2,h3'));headings.forEach((h,i)=>h.id='reading-section-'+i);$('#toc-links').innerHTML=headings.map(h=>`<a href="#${h.id}">${esc(h.textContent)}</a>`).join('')||'<span class="small muted">Нет разделов</span>';UI.readerRendered(current.id,mode);if(deviceId)DB.rpc('log_lecture_view',{p_device_id:deviceId,p_lecture_id:current.id,p_mode:mode,p_ip:currentIp}).catch(()=>{})}
 ...
 async function track(){
  ...
  document.addEventListener('visibilitychange',()=>{
   ...
   if(deviceId)DB.rpc('device_heartbeat',{p_device_id:deviceId,p_active:false,p_ip:currentIp}).catch(()=>{})
  });
 }
 ...
 try{
  ...
  try{await track()}catch(e){if(/blocked|access_denied/.test(e.message)){lock();return}toast('Материалы открыты. Статистика временно недоступна.')}
  if(locked)return;
  const loaded=await Promise.all([
   DB.select('subjects','select=*&order=sort_order.asc,name.asc'),
   DB.select('weeks','select=*&order=sort_order.desc,starts_on.desc'),
   DB.select('lectures','select=id,subject_id,week_id,title,lecture_date,starts_at,pair_number,summary,lecture_text,full_text,assignment_text,published&published=eq.true&order=lecture_date.desc,starts_at.desc&limit=1000')
  ]);
  [subjects,weeks,lectures]=loaded;
  poll=setInterval(async()=>{
   if(document.hidden||locked)return;
   try{const p=await DB.profile();if(p.blocked)lock()}catch{}
  },20000);
